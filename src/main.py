...
from config import THREADS_USERNAME, START_DATE, END_DATE, OUTPUT_DIR
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("분석 결과 요약 (Meta 커뮤니티 규정 기준)")
    print("=" * 70)
    ...
    print(f"반복/중복 게시물: {summary['duplicate_count']}개")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"## Meta 커뮤니티 규정 기준 분석\n\n")
        f.write(f"### 요약\n")
        f.write(f"- 총 게시물: **{summary['total_posts']}개**\n")
        f.write(f"- 🔴 매우 높음 (삭제 가능성): **{summary['critical_count']}개**\n")
        f.write(f"- 🟠 높음 (경고/제한 가능성): **{summary['high_risk_count']}개**\n")
        f.write(f"- 🟡 중간 (주의 필요): **{summary['medium_risk_count']}개**\n")
                ...
                if post["risk_score"] > 0:
                    ...
                    if post.get("violations"):
                        ...
                        for v in post["violations"]:
                            f.write(f"  - {v['category']}/{v['subcategory']}\n")
                    if post.get("recommendations"):
                        f.write(f"- 권고사항: {post['recommendations'][0][:100]}...\n")
                    f.write("\n")
        
        f.write("\n---\n")
        f.write("*이 분석은 Meta 커뮤니티 규정을 기반으로 하며, 실제 Meta의 판단과 다를 수 있습니다.*\n")
